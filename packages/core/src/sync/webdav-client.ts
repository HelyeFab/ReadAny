...
export class WebDavClient {
  ...
  async putFileChunked(
    ...
    if (!mkcol.ok && mkcol.status !== 405) {
      throw new Error(
        `Chunked upload could not start: ${summarizeStatus(mkcol.status, mkcol.statusText)}`,
      );
    }
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        if (!resp.ok) {
          throw new Error(
            `Chunk ${chunk.index + 1}/${chunks.length} failed: ${summarizeStatus(resp.status, resp.statusText)}`,
          );
        }
        loaded += chunk.length;
      ...
      if (!move.ok) {
        throw new Error(
          `Chunked upload could not be assembled: ${summarizeStatus(move.status, move.statusText)}`,
        );
      }
      this.hadAuthSuccess = true;
      console.log(
